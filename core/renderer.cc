...
#define LOGE(...) VFE_LOGE("Renderer", __VA_ARGS__)
#define LOGI(...) VFE_LOGI("Renderer", __VA_ARGS__)
...
VkShaderModule Renderer::loadShader(const char* path) {
  std::vector<uint8_t> bytes = assetLoader(path);
  if (bytes.empty()) {
...
void Renderer::init(VkDevice dev, VkPhysicalDevice phyDev, AssetLoader loader,
                    uint32_t width, uint32_t height) {
  device         = dev;
  physicalDevice = phyDev;
  assetLoader    = std::move(loader);
  screenWidth    = width;
  screenHeight   = height;
...
void Renderer::createMsdfResources(VkRenderPass renderPass, const MsdfFont& font,
  ...
  msdfAtlasW [w] = font.atlasW();
  msdfAtlasH [w] = font.atlasH();
  msdfPxRange[w] = font.distanceRange();
  ...
  img.extent    = {msdfAtlasW[w], msdfAtlasH[w], 1};
  ...
  vkCreateImage(device, &img, nullptr, &msdfAtlasImage[w]);

  VkMemoryRequirements ir{};
  vkGetImageMemoryRequirements(device, msdfAtlasImage[w], &ir);
  ...
  vkAllocateMemory(device, &ia, nullptr, &msdfAtlasMemory[w]);
  vkBindImageMemory(device, msdfAtlasImage[w], msdfAtlasMemory[w], 0);

  VkImageViewCreateInfo iv{};
  iv.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
  iv.image = msdfAtlasImage[w]; iv.viewType = VK_IMAGE_VIEW_TYPE_2D;
  iv.format = VK_FORMAT_R8G8B8A8_UNORM;
  iv.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1};
  vkCreateImageView(device, &iv, nullptr, &msdfAtlasView[w]);
  ...
  vkCreateSampler(device, &sm, nullptr, &msdfSampler[w]);

  // ── Per-weight: staging buffer (for one-shot atlas upload) ────────────────
  VkDeviceSize atlasBytes = (VkDeviceSize)msdfAtlasW[w] * msdfAtlasH[w] * 4;
  ...
  vkCreateBuffer(device, &sb, nullptr, &msdfStaging[w]);
  VkMemoryRequirements sr{};
  vkGetBufferMemoryRequirements(device, msdfStaging[w], &sr);
  ...
  vkAllocateMemory(device, &sa, nullptr, &msdfStagingMem[w]);
  vkBindBufferMemory(device, msdfStaging[w], msdfStagingMem[w], 0);
  void* sp = nullptr;
  vkMapMemory(device, msdfStagingMem[w], 0, atlasBytes, 0, &sp);
  std::memcpy(sp, font.atlas().data(), (size_t)atlasBytes);
  vkUnmapMemory(device, msdfStagingMem[w]);

  // ── Shared: descriptor layout + pool (created once) ──────────────────────
  if (msdfSetLayout == VK_NULL_HANDLE) {
  ...
  das.descriptorPool = msdfPool; das.descriptorSetCount = 1;
  das.pSetLayouts = &msdfSetLayout;
  vkAllocateDescriptorSets(device, &das, &msdfSet[w]);

  VkDescriptorImageInfo di{};
  di.sampler = msdfSampler[w]; di.imageView = msdfAtlasView[w];
  di.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
  VkWriteDescriptorSet wr{};
  wr.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
  wr.dstSet = msdfSet[w]; wr.dstBinding = 0; wr.descriptorCount = 1;
  wr.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  wr.pImageInfo = &di;
  vkUpdateDescriptorSets(device, 1, &wr, 0, nullptr);

  // ── Shared: vertex buffer (created once, split into per-weight regions) ────
  if (msdfVertBuffer == VK_NULL_HANDLE) {
  ...
  if (msdfPipeline == VK_NULL_HANDLE) {
    ...
    gp.pDynamicState = &ds; gp.layout = msdfPipelineLayout;
    gp.renderPass = renderPass; gp.subpass = 0;
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &gp,
                                  nullptr, &msdfPipeline) != VK_SUCCESS) {
      LOGE("Failed to create MSDF pipeline"); msdfPipeline = VK_NULL_HANDLE;
    } else { LOGI("MSDF pipeline created OK"); }
    vkDestroyShaderModule(device, vs, nullptr);
    vkDestroyShaderModule(device, fs, nullptr);
  }
  LOGI("MSDF weight %d: atlas %ux%u ready", w, msdfAtlasW[w], msdfAtlasH[w]);
}

void Renderer::recordAtlasUpload(VkCommandBuffer cmd, int w) {
  if (msdfAtlasImage[w] == VK_NULL_HANDLE) return;
  VkImageMemoryBarrier toDst{};
  toDst.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
  toDst.oldLayout = VK_IMAGE_LAYOUT_UNDEFINED;
  toDst.newLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
  toDst.image = msdfAtlasImage[w];
  ...
  region.imageExtent      = {msdfAtlasW[w], msdfAtlasH[w], 1};
  vkCmdCopyBufferToImage(cmd, msdfStaging[w], msdfAtlasImage[w],
...
void Renderer::uploadGlyphQuads(const float* verts, uint32_t vertCount, int w) {
  if (w < 0 || w >= MAX_FONT_WEIGHTS || !msdfVertMapped) return;
  uint32_t maxPerWeight = MAX_MSDF_VERTS / (uint32_t)MAX_FONT_WEIGHTS;
  if (vertCount > maxPerWeight) vertCount = maxPerWeight;
  size_t byteOffset = (size_t)msdfVertOffset(w) * MSDF_VERT_FLOATS * sizeof(float);
  if (vertCount > 0)
    std::memcpy((char*)msdfVertMapped + byteOffset, verts,
                (size_t)vertCount * MSDF_VERT_FLOATS * sizeof(float));
  msdfVertCount[w] = vertCount;
}
...
void Renderer::drawMsdfRange(VkCommandBuffer cmd, uint32_t firstVert,
  ...
  if (w < 0 || w >= MAX_FONT_WEIGHTS || msdfAtlasImage[w] == VK_NULL_HANDLE) return;
  ...
  float push[8] = {(float)screenWidth, (float)screenHeight, msdfPxRange[w], 0.f,
                   (float)msdfAtlasW[w], (float)msdfAtlasH[w], scrollX, scrollY};
...
void Renderer::cleanup() {
  ...
  for (int wi = 0; wi < MAX_FONT_WEIGHTS; wi++) {
    if (msdfStaging[wi])    vkDestroyBuffer   (device, msdfStaging[wi],    nullptr);
    if (msdfStagingMem[wi]) vkFreeMemory      (device, msdfStagingMem[wi], nullptr);
    if (msdfSampler[wi])    vkDestroySampler  (device, msdfSampler[wi],    nullptr);
    if (msdfAtlasView[wi])  vkDestroyImageView(device, msdfAtlasView[wi],  nullptr);
    if (msdfAtlasImage[wi]) vkDestroyImage    (device, msdfAtlasImage[wi], nullptr);
    if (msdfAtlasMemory[wi])vkFreeMemory      (device, msdfAtlasMemory[wi],nullptr);
  }
}
